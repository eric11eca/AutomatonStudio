...
truth.evaluate = function (p,facts) {
	if (logic.symbolp(p)) {return truth.evalatom(p,facts)}
	if (p[0] == 'not') {return truth.evalnot(p,facts)}
	if (p[0] == 'and') {return truth.evaland(p,facts)}
	if (p[0] == 'or') {return truth.evalor(p,facts)}
	if (p[0] == 'implication') {return truth.evalimplication(p,facts)}
	if (p[0] == 'reduction') {return truth.evalreduction(p,facts)}
	if (p[0] == 'equivalence') {return truth.evalequivalence(p,facts)}
	return false;
}
